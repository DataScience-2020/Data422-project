...
# This code is to deal with NLP analysis, by reading text data from csv files
...
term <- "voice"
stop_list <-c(term, "https", "http", "young", "null", "game", "player", "play")
stop_words <- ""
...
text_df <- read.csv("data/target_games_fellow.csv") #character.txt")

text.to.analyze <- ""
for(n in 1:nrow(text_df['story_line']))
{
	text.to.analyze = paste(text.to.analyze, str_remove_all(text_df[n,'story_line'], stop_words))
}
...
text.to.analyze <- ""
for(n in 1:nrow(text_df['summary']))
{
	text.to.analyze = paste(text.to.analyze, str_remove_all(text_df[n,'summary'], stop_words))
}
